...
const userInfo = require('../config/question-up-user-info');
...
const pageScroll = async (page, data, browser) => {
  ...
  try {
    ...
    if (index !== -1) {
      ...
      // await page.click('.VoteButton--up');
...
const openBrowser = async (data) => {
  const browser = await puppeteer.launch({headless: false, args: [ `--proxy-server=${data.ipPort}` ]});
